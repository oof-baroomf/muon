...
import { app, BrowserWindow, ipcMain, IpcMainEvent, WebContentsView, Menu, MenuItemConstructorOptions, type Rectangle } from 'electron';
import fs from 'fs';
...
ipcMain.on('view:create', (evt, id: string, url: string) => {
  ...
  if (mainWindow) {
    (mainWindow.contentView as unknown as { addChildView(view: WebContentsView): void }).addChildView(view);
  }
...
ipcMain.on('view:destroy', (evt, id: string) => {
  ...
  if (view) {
    if (mainWindow) {
      (mainWindow.contentView as unknown as { removeChildView(view: WebContentsView): void }).removeChildView(view);
    }
