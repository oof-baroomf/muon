...
function applyScalingCSS(browserView, canvasScale) {
    ...
    const inverseScalePercentage = 100 / canvasScale;
    const css = `
        body {
            width: ${inverseScalePercentage}vw;
            height: ${inverseScalePercentage}vh;
...
ipcMain.on('create-view', (event, { id, url, x, y, canvasScale }) => {
    ...
    view.setBounds({
        x: Math.round(x),
        y: Math.round(y),
        width: Math.round(DEFAULT_VIEW_LOGICAL_WIDTH * canvasScale),
        height: Math.round(DEFAULT_VIEW_LOGICAL_HEIGHT * canvasScale)
    });
